...
\Author{\hl{Roney Duarte da Silva} %MDPI: 1. Please carefully check the accuracy of names and affiliations. 2. The email address highlighted is different from the one submitted online at susy.mdpi.com. Please confirm which is correct.
 $^{1,2}$\orcidA{} and João Marcos Salvi Sakamoto $^{1,2,*}$\orcidB{} }
...
\hl{São José dos Campos} %MDPI: please provide english format of city, same as aff. 2. Response: "São José dos Campos" is a proper noun (the name of the city) and does not have a English translation. Please keep it as "São João dos Campos".
 12228-900, \hl{SP,}  %MDPI: Please confirm if keep it, same as aff. 2. Response: "SP" stands for the state of São Paulo. Please keep it in both affiliations, as it follows the standard format: City, State, Zip Code, Country.
 Brazil; roneyddasilva@gmail.com\\
$^{2}$ \quad Division of Photonics, Institute for Advanced Studies (IEAv), \hl{São José dos Campos} 12228-001, \hl{SP,} Brazil}
...
% Keywords
\keyword{\highlighting{fiber} %MDPI: Please confirm case in keywords.
...
\begin{enumerate}
    \item \textbf{\hl{Embedded} %MDPI: Please confirm if the bold formatting in the paper is necessary; if not, please remove it. 
 Intensity-Based Interrogation:} Development of a custom, low-cost, and compact interrogation system that enables DC-coupled acceleration measurements, overcoming the \gls{swap} limitations of commercial spectral interrogators.
    \item \textbf{System-Level Calibration Model:} Proposal of an extended polynomial calibration methodology that compensates for the aggregate thermal non-linearities of the entire optoelectronic chain (source, couplers, and detectors), surpassing the limitations of transducer-only compensation.
    \item \textbf{Rigorous Stability Analysis:} A comprehensive characterization of the sensor's long-term bias instability and \gls{vrw} using Allan Variance, validating its performance against tactical-grade navigation requirements.
\end{enumerate}
...
\centering
		...
		\vspace{6pt}
		\end{adjustwidth}
\caption{\hl{Connections} %MDPI: Please confirm whether the overlapping/incomplete content in this figure affects scientific understanding and if it does, please revise it.
...
\end{equation}
\hl{The} %MDPI: Please check if keep no indent format of paragraph, please check the whole manuscript and confirm.
...
% 	\centering{}\includegraphics[page=24]{main_figure_file_en.pdf}
% \end{figure}
...
\includegraphics[width=\textwidth]{plot_time_elapsed_fbg_production_en.pdf}
	\caption{\hl{Data} %MDPI: Commas are only used for numbers with five or more digits. We removed them in four-digit numbers in the figure. Please confirm. same as other figures.
...
\includegraphics[width=\textwidth]{fbg_acc_4_en.pdf}
	\caption{\hl{The} %MDPI: 1. Figures should be put after their first citations, please confirm positions of all figures. Except for some in order to reduce too much space. 2. To reduce white space, we have not placed it where it is first mentioned, please confirm.
...
This calibration method is described in detail in the works of \cite{Silva2020b,Kuncar2017}. With the parameters obtained in this step, while maintaining a constant temperature, it was possible to perform the bias instability test, presented in the next section. In \cref{sec:thermal_calib_model}, a modification of this method is presented to include thermal compensation.
...
\includegraphics[width=0.9\columnwidth]{uncalibrated_data_en.png}
	\caption{\hl{Data} %MDPI: Please confirm whether an explanation of the colors needs to be added to the figure caption.
 acquired for calibration under controlled temperature conditions. At the top of the graph, the axis that is parallel to the gravity vector is indicated, where the notation $\mathbf{z}\downarrow$ signifies that  the{ respective axis ($\mathbf{z}$) is oriented downward. In this setup, the other two axes are orthogonal to gravity and, ideally, should exhibit zero measurement. However, the acquired data reveals significant cross-axis coupling effects. The subsequent section {presenting the} results demonstrates the capability of the proposed calibration method to effectively correct these parasitic effects.}}	\label{fig:dados_adquiridos_ensaio_temperatura}
\end{figure}
...
\setlength{\tabcolsep}{2.8mm}
	\caption{\hl{Measured} %MDPI: we added a line in the table. Please confirm.
 power values at rest and conditioning circuit components for each channel.\protect\label{tab:-2}}
	...
	\begin{tabular}{cccccc}
		...
		$\basez$            & $\SI{2}{\uW}$                  & $\SI{886}{\kohm}$                              & $\SI{820}{\kohm}$                                & $\SI{153,2}{\nF}$                            & \hl{$100+\SI{47}{\nF}\left(\SI{1.3}{\hertz}\right)$} %MDPI: Please confirm if  keep no space before (.
      \\
		\bottomrule
	\end{tabular}
...
\begin{figure}[H]
	...
	\caption{\hl{Raw} %MDPI: To reduce white space, we have not placed it where it is first mentioned, please confirm.
...
\begin{figure}[H]
 \includegraphics[width=0.95\textwidth]{calibrated_data_acc_4_calibration_test_7_20240806_en.pdf}
	\caption{\hl{Validation} %MDPI: To reduce white space, we have not placed it where it is first mentioned, please confirm.
...
\setlength{\tabcolsep}{11.8mm}
	\caption{\hl{Accelerometer} %MDPI: 1. Please confirm if keep bold and italic in the table. 2. please bold header in the table.
 instability performance parameters.\protect\label{tab:-4}}
	\centering{}%
	\begin{tabular}{ccc}
		\toprule
		\textbf{Axis}         & {\textbf{VRW}} \boldmath{$\left[\si{\meter\per\second\per\sqrt{{\hour}}}\right]$} & \textbf{\emph{Bias} Instability} {\textbf{[mg]}} \\\midrule
		$\mathbf{x}$ & $\num{3.9594}$                                                    & $\num{0.5383}$                                 \\
		$\mathbf{y}$ & $\num{1.9444}$                                                    & $\num{1.1031}$                                 \\
		$\mathbf{z}$ & $\num{2.8702}$                                                    & $\num{1.8719}$                                 \\
		\bottomrule
...
\caption{\hl{Comparison} %MDPI: Please confirm if the bold formatting in the table is necessary; if not, please remove it. 
...
\textbf{Tactical} & \textbf{\SI{0.1}{}--\SI{10.0}{\milli\gravity}} & \textbf{UAVs, Short-range Missiles} \\
Automotive/Industrial & $>$$\SI{10.0}{\milli\gravity}$ & Airbags, Suspension Control \\ \midrule
\textbf{This Work} & \textbf{$<$$\SI{1.9}{\milli\gravity}$} & \textbf{Sounding Rockets/Low-LEO} \\ \bottomrule
\end{tabular}
\end{table}
...
\caption{\hl{Comparisom} %MDPI: Please cite this table in the text and ensure that the first citation of each table appears in numerical order.
...
\textbf{\hl{This Work} %MDPI: Please confirm if the bold formatting in the table is necessary; if not, please remove it. 
} & \textbf{Single Seismic Mass} & \textbf{Diff. Intensity (embedded)} & \textbf{112 \si{\pico\meter\per\gravity}} (differential) & \textbf{DC---440 \si{\hertz}} & \textbf{Thermal Bias Stability (\textless 2 \si{\milli\gravity})} \\ \bottomrule
\end{tabular}%
}
\noindent{\footnotesize{\textit{\hl{Note:} %MDPI: Please confirm if the italics are necessary; if not, please remove them. 
...
\newblock In \emph{Proceedings of the 2025 11th International Conference on Automation, Robotics, and Applications (ICARA)}; IEEE: \hl{Piscataway, NJ, USA,} %MDPI: Newly added information. Please confirm. Same as below.
  2025; pp. 290--299.
...
Kashyap, R.
\newblock {\em Fiber Bragg Gratings}; Academic Press:  \hl{Cambridge, MA, USA,} %We added the location of publisher. Please confirm
 2010; p. 614.
...
\newblock \hl{A triaxial dual FBG accelerometer that can be used for seismic wave detection.} %MDPI: Refs. 21 and 25 are duplicated. Please remove duplicated references and rearrange all the references so that they appear in numerical order. Please ensure that there are no duplicated references.
...
\newblock In Proceedings of the 2011 IEEE Sensors Applications Symposium,  \hl{San Antonio, TX, USA,} 22--24 February \hl{2011.} %MDPI: We removed pp. 123--126. Please confirm this revision.
...
\newblock In Proceedings of the Anais do Congresso Brasileiro de Autom{\'{a}}tica 2020.\hl{ sbabra,  dec 2020.} %MDPI: please replace them with conference address and date.
...
L{\'{o}}pez-Higuera, J.M.
\newblock {\em Handbook of Optical Fibre Sensing Technology}; Wiley:  \hl{Hoboken, NJ, USA,} %We added the location of publisher. Please confirm
 2002; p. 828.
...
\newblock {\em {Rev. Cient.-Period.-Telecomunicacoes}} {\bf 2000}, \hl{1516--2338.} %MDPI: We are sorry but we could not find the required information about this entry. Please add the volume or doi information.
...
Wallin, A.E.E.;  \hl{et~al.} %MDPI: Please write out the first ten authors' names before using “et al.” in the references.
